...
- OpenAI's Terms of Use prohibit automatically or programmatically extracting data or Output and prohibit representing AI Output as human-generated when it was not.
- OpenAI's Terms of Use include restrictions against sharing account credentials or making an account available to others.
- OpenAI's API key safety guidance says not to deploy API keys in client-side environments such as browsers or mobile apps, and recommends routing requests through a backend server.

This document should be reviewed against the latest OpenAI policies before any public release.
